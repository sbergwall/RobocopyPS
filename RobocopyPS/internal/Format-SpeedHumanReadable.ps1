Function Format-SpeedHumanReadable {
    ...
    If ($Unit -eq 'Auto') {
        ...
        switch ($Unit) {
            'PB'  { "{1}{0:#.#' PB'}" -f ($absSize / 1PB), $Operator; break }
            'TB'  { "{1}{0:#.#' TB'}" -f ($absSize / 1TB), $Operator; break }
            'GB'  { "{1}{0:#.#' GB'}" -f ($absSize / 1GB), $Operator; break }
            'MB'  { "{1}{0:#.#' MB'}" -f ($absSize / 1MB), $Operator; break }
            'KB'  { "{1}{0:#' KB'}" -f ($absSize / 1KB), $Operator; break }
            default { "{1}{0}" -f ($absSize), $Operator + " B" }
        }
    }
}